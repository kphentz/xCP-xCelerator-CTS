...
public class MergeVDMNewObject extends DfSingleDocbaseModule
{
    public Result execute (String packageToTransform, boolean newObject, String newObjectName, String newObjectType, String rootPathName, String appendPath, String folderObjectType, boolean createTOC, boolean createBookmarks, boolean enableHyperlinks, boolean optimizeForWeb){
        ...
        try{
			DfLogger.debug(this,"Inside Merging Virtual Document to PDF", null, null);
			session = getSession();
    	    ...
    	    if( objectToTransform == null) {
    	    	String errorMsg = "There isn't an object associated to the package in MergeVDtoPDF";
        	...
        	DfLogger.debug(this,"Create TOC	: " + createTOC, null, null);
        	DfLogger.debug(this,"Create Bookmarks	: " + createBookmarks, null, null);
        	DfLogger.debug(this,"Enable Hyperlinks	: " + enableHyperlinks, null, null);
        	DfLogger.debug(this,"Optimize for Web: " + optimizeForWeb, null, null);
        	if(newObject){
			...
			DfLogger.debug(this,"Finding profiles to Merge Virtual Document",  null, null);
        	...
        	if(mediaProfiles != null){
    			...
    			for(;itr.hasNext();){
    				...
    				if( profile != null) {
    					...
    					IProfileParameter theProfileParameters[] = profile.getParameters();
    					DfLogger.debug(this, "Setting Profile Parameters", null, null);
    			    	theProfileParameters = this.setMergeVDMProfileParameters( theProfileParameters, createTOC, createBookmarks, enableHyperlinks, optimizeForWeb);
    			...
    			DfLogger.debug(this, "Formats not supported. Profile not found", null, null);
    			result.setErrorMessage(errorMsg);
    			result.setSuccess(false);
    			return result;
    		}
        	DfLogger.debug(this, "Virtual Document merged to PDF Successfully", null, null);
    	    return result;
        }catch (DfException e){
        	String errorMsg = "DFC Error in Merging Virtual Document";
            DfLogger.error(this, errorMsg, null, e);
            result.setErrorMessage(errorMsg);
            result.setSuccess(false);
        }catch(Exception e){
        	String errorMsg = "Error in Merging Virtual Document";
	        						...
	        						for( int k = 0; k <proFilters.length ;k++){
	        							...
	        							for(int x = 0; x<filterValues.length; x++){
	        								...
	        								if(filterName != null && filterValue != null){
	        									if(filterName.equalsIgnoreCase("RenditonType") && filterValue.equalsIgnoreCase("vdtoPDF")){
    ...
    private IProfileParameter[] setMergeVDMProfileParameters(IProfileParameter aiprofileparameter[], boolean flag, boolean flag1, boolean flag2, boolean flag3){
        setProfileParameter(aiprofileparameter, "doc_token_changeDefaultSettings", "Yes");
        setProfileParameter(aiprofileparameter, "doc_token_pdfVersion", "PDFVersion16");
        setProfileParameter(aiprofileparameter, "doc_token_resolution", "300");
        setProfileParameter(aiprofileparameter, "doc_token_enableSecurity", "No");
        setProfileParameter(aiprofileparameter, "doc_token_encryptionMode", "40bit");
        setProfileParameter(aiprofileparameter, "doc_token_enableAccess", "Disabled");
        setProfileParameter(aiprofileparameter, "doc_token_allowCopy", "Disabled");
        setProfileParameter(aiprofileparameter, "doc_token_changesAllowed", "Disabled");
        setProfileParameter(aiprofileparameter, "doc_token_printing", "Disabled");
        setProfileParameter(aiprofileparameter, "doc_token_docAssembly", "Disabled");
        setProfileParameter(aiprofileparameter, "doc_token_formFieldFilling", "Disabled");
        setProfileParameter(aiprofileparameter, "doc_token_secOpass", "passopen");
        setProfileParameter(aiprofileparameter, "doc_token_secCpass", "passchange");
        if (flag) {
            setProfileParameter(aiprofileparameter, "doc_token_toc", "Yes");            
        } else {
            setProfileParameter(aiprofileparameter, "doc_token_toc", "No");            
        }
        if (flag1) {
            setProfileParameter(aiprofileparameter, "doc_token_enableBookMarks", "Yes");
        } else {
            setProfileParameter(aiprofileparameter, "doc_token_enableBookMarks", "No");
        }
        if (flag2) {
            setProfileParameter(aiprofileparameter, "doc_token_enableHyperlinks", "Yes");
        } else {
            setProfileParameter(aiprofileparameter, "doc_token_enableHyperlinks", "No");
        }
        if (flag3) {
            setProfileParameter(aiprofileparameter, "doc_token_optimize", "Yes");
        } else {
            setProfileParameter(aiprofileparameter, "doc_token_optimize", "No");
        }
        return aiprofileparameter;        
    }


    private void setProfileParameter(IProfileParameter aiprofileparameter[], String s, String s1) {
        int i = 0;
        for (boolean flag = false; i < aiprofileparameter.length && !flag; i++) {
             if (aiprofileparameter[i].getParameterName().compareTo(s) == 0) {
                 aiprofileparameter[i].setParameterValue(s1);
                 flag = true;
             }             
        }        
    }
